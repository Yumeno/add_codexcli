...
- **git でレビュー・巻き戻し**: 通常の tracked / untracked の未コミット変更は `git diff` で
  確認でき、`git checkout -- .` && `git clean -fd` で破棄できる。一方、git 履歴の操作、
  ignored ファイル（`.env` 等）、リポジトリ外への変更、外部サービスへの副作用は git では
  戻せないため、別途確認が必要（スキル側では Codex への仕様書で git 操作・保護対象ファイルへの
  接触を禁止し、実行後に HEAD 比較で逸脱を検出する）
- **検収フロー**: 実行後に HEAD / branch の実行前後比較、`git status --porcelain=v1
  --untracked-files=all` / `git diff HEAD --stat` と、Codex 自身が報告した変更ファイル一覧を
  突き合わせ、不一致があれば警告する
