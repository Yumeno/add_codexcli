...
Claude Code 側の SKILL.md は `~/.claude/scripts/` を直接参照するため、
グローバルインストールを推奨します。プロジェクトローカル運用は SKILL.md 内の
wrapper パスを手動で書き換える必要があるため、次項の「プロジェクトローカル」を参照してください。
...
- `codex-wrapper.ps1` / `codex-wrapper.sh`
- `codex-verify.ps1` / `codex-verify.sh`
- `list-codex-models.ps1` / `list-codex-models.sh`

> **注記:** Claude Code 向け専用のインストーラーは現在準備中です ([#27](https://github.com/Yumeno/add_codexcli/issues/27))。
> 完成までは以下の手動手順を使ってください。
...
リポジトリ内の `.claude/skills/` と `scripts/` がそのまま使われます。

#### 方法 2: 個人スキルとしてグローバルインストール（推奨）

特定のプロジェクトではなく、すべてのプロジェクトで使いたい場合の推奨手順です。
5 skill と 6 scripts をリポジトリの正本からコピーします。

**Linux / macOS / WSL / Git Bash:**

```bash
# リポジトリをクローン (作業ディレクトリはどこでも可)
git clone https://github.com/Yumeno/add_codexcli.git /tmp/add_codexcli
cd /tmp/add_codexcli

# 5 skill をグローバルにコピー
mkdir -p ~/.claude/skills
for name in ask-codex ask-codex-with-context codex-implement list-codex-models set-codex-model; do
    mkdir -p "$HOME/.claude/skills/$name"
    cp ".claude/skills/$name/SKILL.md" "$HOME/.claude/skills/$name/SKILL.md"
done

# 6 scripts をグローバルにコピー
mkdir -p ~/.claude/scripts
cp scripts/codex-wrapper.ps1 ~/.claude/scripts/
cp scripts/codex-wrapper.sh ~/.claude/scripts/
cp scripts/codex-verify.ps1 ~/.claude/scripts/
cp scripts/codex-verify.sh ~/.claude/scripts/
cp scripts/list-codex-models.ps1 ~/.claude/scripts/
cp scripts/list-codex-models.sh ~/.claude/scripts/
chmod +x ~/.claude/scripts/codex-wrapper.sh
chmod +x ~/.claude/scripts/codex-verify.sh
chmod +x ~/.claude/scripts/list-codex-models.sh
```
...
```powershell
# リポジトリをクローン
git clone https://github.com/Yumeno/add_codexcli.git $env:TEMP\add_codexcli
Set-Location $env:TEMP\add_codexcli

# 5 skill をグローバルにコピー
$ClaudeDir = "$env:USERPROFILE\.claude"
New-Item -ItemType Directory -Force -Path "$ClaudeDir\skills", "$ClaudeDir\scripts" | Out-Null
foreach ($name in @('ask-codex','ask-codex-with-context','codex-implement','list-codex-models','set-codex-model')) {
    New-Item -ItemType Directory -Force -Path "$ClaudeDir\skills\$name" | Out-Null
    Copy-Item -LiteralPath ".claude\skills\$name\SKILL.md" -Destination "$ClaudeDir\skills\$name\SKILL.md" -Force
}

# 6 scripts をグローバルにコピー
foreach ($name in @('codex-wrapper.ps1','codex-wrapper.sh','codex-verify.ps1','codex-verify.sh','list-codex-models.ps1','list-codex-models.sh')) {
    Copy-Item -LiteralPath "scripts\$name" -Destination "$ClaudeDir\scripts\$name" -Force
}
```

インストール後、Claude Code を再起動すると `/ask-codex`、`/ask-codex-with-context`、
`/codex-implement`、`/list-codex-models`、`/set-codex-model` が slash command として使えます。

現行の `~/.claude/skills/<name>/SKILL.md` は `~/.claude/scripts/` を直接参照するため、
グローバルインストールでは追加の path 書き換えは不要です。

#### 方法 3: プロジェクトローカルインストール
...
# 5 skill をプロジェクトローカルにコピー
mkdir -p .claude/skills
for name in ask-codex ask-codex-with-context codex-implement list-codex-models set-codex-model; do
    mkdir -p ".claude/skills/$name"
    curl -sL "https://raw.githubusercontent.com/Yumeno/add_codexcli/main/.claude/skills/$name/SKILL.md" \
        -o ".claude/skills/$name/SKILL.md"
done

# 6 scripts をプロジェクトの scripts/ にコピー
mkdir -p scripts
for name in codex-wrapper.ps1 codex-wrapper.sh codex-verify.ps1 codex-verify.sh list-codex-models.ps1 list-codex-models.sh; do
    curl -sL "https://raw.githubusercontent.com/Yumeno/add_codexcli/main/scripts/$name" \
        -o "scripts/$name"
done
chmod +x scripts/codex-wrapper.sh scripts/codex-verify.sh scripts/list-codex-models.sh
```

> **重要（プロジェクトローカル運用の制約）:** 現行の `SKILL.md` は `~/.claude/scripts/` を
> 参照するように書かれているため、そのままだとプロジェクト内の `scripts/` を見に行きません。
> プロジェクトローカルで使う場合は、コピーした各 SKILL.md 内の `$HOME/.claude/scripts/`
> という記述を `${PWD}/scripts/` などのプロジェクトパスへ手動で書き換える必要があります。
> この不便を解消するインストーラーを [#27](https://github.com/Yumeno/add_codexcli/issues/27) で用意予定です。
