...
Codex CLIは画像生成を内包しており、このスキルのフローのまま画像の生成・編集を委任できる。仕様書を書く前に、この `SKILL.md` のディレクトリ直下の `references/image-generation.md` を読む。
...
仕様書にファイルパスを書くことは、そのファイルの内容をモデルへ直接添付することとは異なる。Codexが必要に応じて、実行環境のツールでファイルを読み取り・解析する。

画像生成・編集では、仕様書に編集元画像のパスを書くことで、Codexが利用可能な画像ツールへの参照入力を組み立てる。この経路と、wrapperのAttachmentからCodex CLIの`-i`へ渡すVLM入力(`ask-codex-with-context`用)は区別する。

PDFやOffice文書等の解析では、利用可能なライブラリ・ツールとアクセス権に依存する。音声・動画についても、文字起こしやフレーム抽出等の処理が必要になる場合がある。パスを示しただけでネイティブなマルチモーダル入力が成立したとは報告しない。
