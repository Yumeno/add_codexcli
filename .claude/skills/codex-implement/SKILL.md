...
組み立て方 (生成・編集それぞれ)、モデル名非可視の原則、検収方法、複数枚生成時の
出力先の縛り方を記載している。

要点のみ抜粋:

- 仕様書には **ファイル名・形式・被写体/内容・スタイル・避けたい要素** を書く。
  細かい構図・照明などのプロンプトエンジニアリングは Codex が自動で行う
- 編集は **入力画像のリポジトリ内パスを仕様書に書くだけでよい** (`-Attachment` 不要)。
  「変更点の限定列挙 + 維持項目の明示列挙」で指示する
- **生成モデル名は応答にもログにも出ない**。モデル名を報告させない・推測で書かない
  (テキスト回答のモデル名フッター規則と同じ原則)
- **出力先はリポジトリ内に縛る** (リポジトリ外への書き出しは git 検収に映らない)。
  枚数・ファイル名が事前に不明な生成でも、検収 (差分検出型) はそのまま機能する
- 検収では生成物の magic bytes を確認する (PNG は先頭 8 バイト `89 50 4E 47 0D 0A 1A 0A`)
