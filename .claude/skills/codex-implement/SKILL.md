...
## 非コード成果物 (画像アセット等) の生成・編集

Codex CLI は画像生成 (feature flag `image_generation`, stable) を内包しており、
このスキルのフロー (仕様書 → wrapper → 検収) のまま画像アセットの生成・編集を委任できる。

画像タスクの仕様書を書く前に、このスキル同梱の `references/image-generation.md`
(この SKILL.md と同じディレクトリの `references/` 配下) を読むこと。プロンプトの
組み立て方 (生成・編集それぞれ)、モデル名の取得不可原則、検収方法、複数枚生成時の
出力先の縛り方を記載している。

要点のみ抜粋:

- 画像タスクの仕様書作成前に、同梱の `references/image-generation.md` を読む
- 用途、保存先、内容、スタイル、必須条件、補完してよい項目、検収条件を整理する
- 編集では入力画像、変更点、保持条件、出力先を明示する。変更条件と保持条件を矛盾させない
- API仕様とCodex CLI経由の利用可否を区別する。未公開の引数や推測の画像モデル名を記載しない
- 透明背景は旧世代の制約で一律に諦めず、利用経路の対応を確認し、出力のアルファ値を検収する
- 反復編集では採用済み画像を入力にし、重要な保持条件を再掲する
- 出力先をリポジトリ内に限定し、原画像は原則として別名保存で保持する
- magic bytesとgit差分に加え、画像のデコード、寸法、内容、透過、文字、意図しない変更を確認する

### ファイルパス指定と画像添付の違い

- 仕様書にファイルパスを書くことは、そのファイルの内容をモデルへ直接添付することとは異なる。
  Codex が必要に応じて、実行環境のツールでファイルを読み取り・解析する
- 画像生成・編集では、仕様書に編集元画像のパスを書くことで、Codex が利用可能な画像ツールへの
  参照入力を組み立てる。この経路と、wrapper の `-Attachment` から Codex CLI の `-i` へ渡す
  VLM 入力 (`/ask-codex-with-context` 用) は区別する
- PDF や Office 文書等の解析では、利用可能なライブラリ・ツールとアクセス権に依存する。音声・動画に
  ついても、文字起こしやフレーム抽出等の処理が必要になる場合がある。パスを示しただけで
  ネイティブなマルチモーダル入力が成立したとは報告しない
- 変換ファイルを作る場合も、既存の作業範囲・保存先・検収規約を守り、元の入力ファイルを無断で上書きしない
